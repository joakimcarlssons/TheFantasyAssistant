        ...
        => data.PlayersToTarget.Select(player =>
            new ContentBuilder()
                .AppendCustomText(() => new ContentBuilder()
                    ...
                    .AppendTextWithLineBreak($"{Emoji.BlackSmallSquare} Shots/90 min: {player.ShotsPer90}")
                    .AppendTextWithLineBreak($"{Emoji.BlackSmallSquare} Shots on target/90 min: {player.ShotsOnTargetPer90}")
